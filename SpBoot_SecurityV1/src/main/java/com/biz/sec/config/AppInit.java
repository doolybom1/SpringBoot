...
public class AppInit implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		/*
		 * Optional 클래스로 vo클래스를 wrapping했을때 vo에 담긴
		 * 데이터가 있으면 isPresent()가 true가 된다
		 * 
		 * 데이터가 있으면 다음 명령을 실행하지 않고 return 해서 끝내라
		 */
		Optional<UserVO> vo = uDao.findByUsername("dooly");
		if(vo.isPresent()) {
			return ;
		}
